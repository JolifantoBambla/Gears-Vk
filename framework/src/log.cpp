...
namespace gvk
{
	...
	{
		char buf[256];
    // TODO: check if this does the same as springf_s
	...
	std::string to_string(const glm::vec2& pVector)
	{
		char buf[64];
    // TODO: check if this does the same as springf_s
    std::snprintf(buf, 64, "(%.2f, %.2f)", pVector.x, pVector.y);
		return buf;
	}
	...
	std::string to_string(const glm::vec3& pVector)
	{
		char buf[64];
    // TODO: check if this does the same as springf_s
    std::snprintf(buf, 64, "(%.2f, %.2f, %.2f)", pVector.x, pVector.y, pVector.z);
		return buf;
	}
	...
	std::string to_string(const glm::vec4& pVector)
	{
		char buf[64];
    // TODO: check if this does the same as springf_s
    std::snprintf(buf, 64, "(%.2f, %.2f, %.2f, %.2f)", pVector.x, pVector.y, pVector.z, pVector.w);
		return buf;
	}
